...
#define CYCLE_DELAY         0           // Délai entre chaque cycle (en millisecondes)
...
void loop() {
  ...
  for (int i = 0; i < NUM_LEDS; i++) {
      ...
      FADE_SPEED = constrain(FADE_SPEED, 40, 80);
    ...
    for (int j = 0; j < trailLength; j++) {
      ...
      leds[ledIndex] = CRGB(brightness * TRAIL_BRIGHTNESS_FACTOR, brightness * TRAIL_BRIGHTNESS_FACTOR, 255); // Couleur bleue avec luminosité réduite
      leds[ledIndex2] = CRGB(brightness * TRAIL_BRIGHTNESS_FACTOR, brightness * TRAIL_BRIGHTNESS_FACTOR, 255); // Couleur bleue avec luminosité réduite
      leds[ledIndex3] = CRGB(brightness * TRAIL_BRIGHTNESS_FACTOR, brightness * TRAIL_BRIGHTNESS_FACTOR, 255); // Couleur bleue avec luminosité réduite
    }
    
    // Allumer le pixel principal
    leds[i] = CRGB(MAX_BRIGHTNESS, MAX_BRIGHTNESS, 255); // Couleur bleue avec luminosité maximale
    leds[i + 30] = CRGB(MAX_BRIGHTNESS, MAX_BRIGHTNESS, 255); // Couleur bleue avec luminosité maximale
    leds[i + 60] = CRGB(MAX_BRIGHTNESS, MAX_BRIGHTNESS, 255); // Couleur bleue avec luminosité maximale
    FastLED.show();
    delay(FADE_SPEED);
