...
const char* esp32_id = "Peluche9"; // Identifiant unique pour cet ESP32
...
void gameLogicTask(void* parameter) {
  while (true) {
    ...
    Serial.println(distance);
      ...
      if (millis() - LastSpot > (vitDetection * vitDiff) && distance > 2 && distance < 200) {
        LastSpot = millis();        
        led += 1;
        CurrentTime = millis();
        End = millis();
      ...
      if (millis() - LastSpot > (vitDetection * vitDiff) && distance > 2 && distance < 200 && closestPlayer == 0){
        LastSpot = millis();
