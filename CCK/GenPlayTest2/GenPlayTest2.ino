...
void mgUpdateLED() {
  ...
  int pin = MG_PORTS[mgTargetIdx];
  ReglageOK = (digitalRead(pin) == LOW);  // pince (GND) -> LOW si bon pin
  if (ReglageOK && !JustOnce2) {
    audioStop();
    // audioPlay();  // piste réparation
    JustOnce2 = true;
    if (genState == COUNTING) {
      audioLoop(10);
    }
    if (genState == COUNTING2) {
      // audioLoop(4);  // piste phase 2
    }
  }
...
inline float clamp01(float x){ if(x<0) return 0; if(x>1) return 1; return x; }
    ...
    while (xQueueReceive(audioQ, &cmd, 0) == pdTRUE) {
      switch (cmd.type) {
        case AUD_PLAY:
          ...
          else { dfp_next = (int)cmd.track; dfp_next_loop = false; }
          break;

        case AUD_LOOP:
          if (!dfp_playing) dfpStartNow(cmd.track, true);
          else { dfp_next = (int)cmd.track; dfp_next_loop = true; }
          break;
...
void updateProgressRing(int turnCount, int totalTours) {
  ...
  // --- params ajustables ---
  const uint16_t H_RED   = 0;                                // 0°
  const uint16_t H_GREEN = (uint16_t)(105.0f/360.0f*65535);  // ~vert
  const uint8_t  SAT     = 230;                              // saturation fixe
  const uint8_t  V_RED   = 40;                               // rouge "reste" sombre
  const uint8_t  V_GREEN_BASE = 110;                         // base verte
  const uint8_t  V_GREEN_PEAK = 220;                         // pic respiration
  const uint8_t  V_SWEEP_HEAD = 255;                         // tête du balayage
  const uint8_t  V_SWEEP_T1   = 180;                         // trainée 1
  const uint8_t  V_SWEEP_T2   = 120;                         // trainée 2
  const uint16_t SWEEP_MS     = 55;                          // vitesse balayage

  totalTours = max(totalTours, 1);
  float targetPct = (float)turnCount / (float)totalTours;
  targetPct = constrain(targetPct, 0.0f, 1.0f);
  ...
  // respiration sur la partie verte (0.6..1.0)
  float breathe = 0.60f + 0.40f * sinf(now * 0.006f);
  uint8_t vGreen = (uint8_t)mixf((float)V_GREEN_BASE, (float)V_GREEN_PEAK, breathe);

  // fond global : rouge sombre partout (reste à faire lisible mais discret)
  for (int i = start; i < end; ++i) {
    ring.setPixelColor(i, HSV(H_RED, SAT, V_RED));
  }

  // partie remplie : vert respirant
  for (int i = start; i < start + filled; ++i) {
    ring.setPixelColor(i, HSV(H_GREEN, SAT, vGreen));
  }

  // balayage qui tourne UNIQUEMENT dans la zone remplie (si > 0)
  if (filled > 0) {
    if (now - fx1.tSweep >= SWEEP_MS) {
      fx1.tSweep = now;
      fx1.sweep = (fx1.sweep + 1) % filled;    // avance dans [0..filled-1]
    }
    int head = start + fx1.sweep;
    int t1   = head - 1; if (t1 < start) t1 += filled;  // wrap dans la zone remplie
    int t2   = head - 2; if (t2 < start) t2 += filled;

    ring.setPixelColor(head, HSV(H_GREEN, SAT, V_SWEEP_HEAD));
    ring.setPixelColor(t1,   HSV(H_GREEN, SAT, V_SWEEP_T1));
    ring.setPixelColor(t2,   HSV(H_GREEN, SAT, V_SWEEP_T2));
  }
...
void updateProgressRing2(int turnCount, int totalTours) {
  ...
  // mêmes paramètres pour cohérence visuelle
  const uint16_t H_RED   = 0;
  const uint16_t H_GREEN = (uint16_t)(105.0f/360.0f*65535);
  const uint8_t  SAT     = 230;
  const uint8_t  V_RED   = 40;
  const uint8_t  V_GREEN_BASE = 110;
  const uint8_t  V_GREEN_PEAK = 220;
  const uint8_t  V_SWEEP_HEAD = 255;
  const uint8_t  V_SWEEP_T1   = 180;
  const uint8_t  V_SWEEP_T2   = 120;
  const uint16_t SWEEP_MS     = 55;

  totalTours = max(totalTours, 1);
  float targetPct = (float)turnCount / (float)totalTours;
  targetPct = constrain(targetPct, 0.0f, 1.0f);

  fx2.pctSmooth = mixf(fx2.pctSmooth, targetPct, 0.18f);
  int filled = (int)floorf(fx2.pctSmooth * span);
  if (filled < 0) filled = 0;
  if (filled > span) filled = span;

  float breathe = 0.60f + 0.40f * sinf(now * 0.006f + 1.4f); // léger déphasage
  uint8_t vGreen = (uint8_t)mixf((float)V_GREEN_BASE, (float)V_GREEN_PEAK, breathe);
  ...
  if (filled > 0) {
    if (now - fx2.tSweep >= SWEEP_MS) {
      fx2.tSweep = now;
      fx2.sweep = (fx2.sweep + 1) % filled;
    }
    int head = start + fx2.sweep;
    int t1   = head - 1; if (t1 < start) t1 += filled;
    int t2   = head - 2; if (t2 < start) t2 += filled;
