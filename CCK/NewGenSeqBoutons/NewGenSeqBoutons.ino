...
// --- Debounce ReglageOK (anti-rebond) ---
static bool ReglageOK_filt = true;      // état "propre" utilisé par le gameplay/audio
static bool ReglageOK_raw_prev = true;  // dernier raw vu
static unsigned long regl_raw_change_ms = 0;

static const unsigned long REGLAGE_DEBOUNCE_MS = 400;  // ajuste (150..400ms typique)
...
static inline void debounceReglageOK(bool raw) {
  unsigned long now = millis();

  // si le raw change, on redémarre le chrono de stabilité
  if (raw != ReglageOK_raw_prev) {
    ReglageOK_raw_prev = raw;
    regl_raw_change_ms = now;
  }

  // si le raw est stable depuis assez longtemps, on l'accepte
  if (raw != ReglageOK_filt && (now - regl_raw_change_ms) >= REGLAGE_DEBOUNCE_MS) {
    ReglageOK_filt = raw;
  }
}
...
void sabotageAllMiniGames() {
  // On relance une cible pour CHAQUE mini-jeu
  pickNewTarget(MG_CROC);
  pickNewTarget(MG_SWITCHES);
  pickNewTarget(MG_POTS);

  // On force une réévaluation immédiate des états OK / pas OK
  updateAllMiniGames();

  // On rouvre les cues audio liés au réglage
  JustOnce = false;

  Serial.println("[SABOTAGE] Nouvelles cibles pour tous les mini-jeux");
}

// Aligne les cibles sur les réglages ACTUELS pour démarrer avec "tout bon"
void syncTargetsToCurrent(bool forceOK = true) {
  unsigned long now = millis();
  ...
  bool s1 = readSwitchPin(SW_PINS[1]);
  bool s2 = readSwitchPin(SW_PINS[2]);
  swTarget = (byte)((s2 << 2) | (s1 << 1) | (s0 << 0));

  swInMatch = true;
  swMatchSince = now - MATCH_HOLD_MS;
  swOK = true;
  ...
  updateAllMiniGames();      // recalc cohérent avec tes règles
...
void generatorTask(void* parameter) {
  ...
  while (true) {
    ...
    if (anyThreat(-60) && !Shocked) {  // Vérifie que le joueur est détecté ET à moins de 5m
    ...
    switch (genState) {
      case COUNTING:
        ...
        updateAllMiniGames();
        debounceReglageOK(ReglageOK);  // met à jour ReglageOK_filt
        ...
        if (capteur2Passe && digitalRead(MAG_SENSOR_2) == 0) {  // Vérifie si capteur 2 a été activé avant
          ...
          if (!ReglageOK) {
            ...
            if (randomTurn > RegFalse) {
              ...
              int randomBadTurn = esp_random() % 5;
              if (randomBadTurn < RegFalse) turnCount -= 1;
              // audioStop();
              // audioPlay(13);
              audioLoop(14);
            }
      ...
      case WAITING:
        {
          ...
          if (RegFalse > 0 && millis() - SaboteTime > 1000) {
            SaboteTime = millis();
            int randomPick = esp_random() % 4;
            if (randomPick + 1 <= RegFalse) {
              durationMs += 1000;
            }
          }
          ...
          if (elapsed >= durationMs) {
            ...
            // mgHasTarget = false;
            audioStop();
            if (RegFalse == 0) audioLoop(10);
            else if (RegFalse > 0) audioLoop(14);
            JustOnce = false;
      ...
      case COUNTING2:
        {
          ...
          if (turnCount >= (NbrTour) + (diffLvl * 2)) {
            genState = WAITP3;
      ...
      case WAITP3:
        {
          ...
          if (elapsed >= durationMs && Phase3Bool) {
            genState = PHASE3;
            // stopLoopTrack();
            audioStop();
            lastTurnDetectedTime = millis();  // Mise à jour de l'heure du dernier tour détecté
      ...
      case PHASE3:
        {
          ...
          if (buttonPressed && !buttonWasPressed && RegFalse == 0) {
          ...
          if (!buttonPressed && RegFalse == 0) {
            ...
            if (RegFalse == 0) {
              ...
              static unsigned long lastFlickerUpdate = 0;
              const unsigned long flickerInterval = 30;  // ms → grésillement nerveux

              if (currentMillis - lastFlickerUpdate >= flickerInterval) {
                lastFlickerUpdate = currentMillis;

                // luminosité de base
                const int baseBrightness = 160;

                // amplitude du grésillement (augmente avec RegFalse)
                int flickerAmplitude = constrain(RegFalse * 20, 20, 120);

                // variation pseudo-aléatoire
                int flicker = random(-flickerAmplitude, flickerAmplitude);

                int duty = constrain(baseBrightness + flicker, 0, 255);
                analogWrite(LED_BUTTON_PIN, duty);
              }
            }
          }
...
void applyDifficultyKeepPercent(float newLvl) {
  ...
  if (genState == WAITING) {
    unsigned long now = millis();
    unsigned long oldDur = (unsigned long)(18000.0f * oldLvl);
    unsigned long newDur = (unsigned long)(18000.0f * newLvl);
  ...
  if (genState == WAITP3) {
    unsigned long now = millis();
    unsigned long oldDur = (unsigned long)(12000.0f * oldLvl);
    unsigned long newDur = (unsigned long)(12000.0f * newLvl);
    if (oldDur == 0) oldDur = 1;
    float pct = (float)(now - stateStartTime) / (float)oldDur;
    pct = constrain(pct, 0.0f, 1.0f);
    stateStartTime = now - (unsigned long)(pct * (float)newDur);
