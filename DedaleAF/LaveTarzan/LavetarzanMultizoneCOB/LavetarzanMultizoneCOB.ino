...
#define PIN                 6
#define NUMPIXELS_PER_STRIP 300
#define NUM_STRIPS         1
...
void loop() {
  ...
  for (int i = 0; i < NUM_STRIPS; i++) {
    ...
    if (currentMillis - temp[i] > interval[i] && g[i] > y[i] - vitesseUpAnim && g[i] < y[i] + vitesseDownAnim) {
      int r = random(RED);
      if (r == 0) {
        y[i] = random((valeurGreen * fluiditeAnim) + 1);
      ...
      g[i] += z[i];
      g[i] = constrain(g[i], 0, valeurGreen * fluiditeAnim);
    } else if (g[i] > y[i]) {
      g[i] -= x[i];
      g[i] = constrain(g[i], 0, valeurGreen * fluiditeAnim);
    }

    fillStrip(i, CRGB(255,constrain(g[i] / fluiditeAnim,0,255),0));
  }
  FastLED.show();
  delay(delayGlobal);
}
